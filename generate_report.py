...
def set_run_font(run, font_name='宋体', size=None, bold=False, color=None):
    ...
    run.element.rPr.rFonts.set(qn('w:ascii'), font_name)
    run.element.rPr.rFonts.set(qn('w:hAnsi'), font_name)
    if size:
        run.font.size = size
    run.bold = bold
    if color:
...
style.element.rPr.rFonts.set(qn('w:ascii'), '宋体')
style.element.rPr.rFonts.set(qn('w:hAnsi'), '宋体')
...
for i in range(1, 4):
    ...
    h_style.element.rPr.rFonts.set(qn('w:ascii'), '宋体')
    h_style.element.rPr.rFonts.set(qn('w:hAnsi'), '宋体')
    if i == 1:
        h_font.size = Pt(16)
    elif i == 2:
        h_font.size = Pt(14)
...
def add_code_block(doc, code_text, title=None):
    ...
    if title:
        p = doc.add_paragraph()
        run = p.add_run(title)
        set_run_font(run, '宋体', Pt(11), bold=True)
...
set_run_font(run, '宋体', Pt(26), bold=True)
...
p.alignment = WD_ALIGN_PARAGRAPH.CENTER
run = p.add_run('目  录')
set_run_font(run, '宋体', Pt(18), bold=True)
...
for level, text in toc_entries:
    ...
    if level == 0:
        set_run_font(run_title, '宋体', Pt(12), bold=True)
    else:
        set_run_font(run_title, '宋体', Pt(11))

    # 制表符 + 点前导符（用省略号模拟）
    run_dots = p.add_run('  ' + '·' * 40 + '  ')
    set_run_font(run_dots, '宋体', Pt(10))
